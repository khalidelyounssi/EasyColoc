            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-separate border-spacing-y-4">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-[#FAFAFA] rounded-[2.5rem] group hover:bg-white hover:shadow-lg transition-all duration-300 border border-transparent hover:border-gray-50">
                            <td class="px-8 py-8">
                            ...
                            <td class="px-8 py-8 text-right">
                                @if($user->id !== auth()->id())
                                    <form action="{{ route('admin.users.toggle-ban', $user->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="bg-white border border-gray-100 px-8 py-3 rounded-full text-[9px] font-black uppercase italic tracking-widest hover:bg-black hover:text-white transition-all shadow-sm">
                                            {{ $user->is_banned ? 'Restore Access' : 'Restrict Access' }}
                                        </button>
                                    </form>
                                @else
                                    <span class="text-[10px] font-bold text-gray-300 uppercase italic px-8 tracking-widest">Root Admin</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
